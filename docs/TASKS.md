...
- [x] **T-05B** **Đóng vòng lặp ván** — kết thúc ván, màn hình kết quả, chơi lại tại chỗ — @Kiet · 03/09
  > Hạng mục **bổ sung ngoài kế hoạch gốc**, khoảng trống thứ sáu sau T-10B, T-14B, T-15B, T-18B, T-15C. T-05 dựng được điều kiện thắng thua và T-49 là màn thống kê sau ván, nhưng **không có hạng mục nào cho việc ván kết thúc rồi chơi lại được**.
  > **Triệu chứng trước khi sửa:** `RunManager.RunEnded` không có một người nghe nào. `EndRun` chỉ đặt `_state = Defeat` rồi thôi — sân đứng im, không màn hình, không lối ra. Cách duy nhất chơi lại là tắt play mode. Trong một buổi kiểm thử tôi vấp phải chuyện này khoảng tám lần, và **T-53 cần ba mươi lượt chơi liên tiếp** để lấy số liệu khoá luận.
  > `Core/RunManager.cs` (`RestartRun`, `CmdRequestRestart`, `RunStarted`, `WavesCleared`, `IsOver`) · `Player/PlayerHealth.ServerRestore` · `Player/PlayerMovement.ServerRespawn` · `UI/RunEndScreen.cs` · chữ pixel `Art/Sprites/UI_*.png` · trình sinh `docs/tools/make_words.py`.
  > **Chơi lại không nạp lại scene.** Nạp lại scene trong Mirror đồng nghĩa với ngắt và nối lại toàn bộ đối tượng mạng, tức là ở co-op người kia bị đá ra rồi phải vào lại. Ở đây chỉ trạng thái được đặt lại, đối tượng mạng giữ nguyên, nên người chơi thứ hai không nhận ra gì ngoài việc ván mới bắt đầu.
  > **Đi đúng mẫu ở mục 3.2:** client gọi `CmdRequestRestart`, host kiểm tra ván đã kết thúc chưa rồi mới thi hành, `SyncVar` tự lan xuống. Host cũng đi qua đúng con đường đó nên không có nhánh riêng cho chơi đơn — mục 3.1. Không đặt quyền chơi lại ở client vì hai người bấm cùng lúc sẽ khởi động hai ván chồng nhau.
  > **Đưa người chơi về điểm sinh phải qua `ClientRpc`.** `NetworkTransform` của người chơi chạy theo chiều client → server, nên vị trí do host ghi bị chính chủ sở hữu ghi đè ngay gói tin kế tiếp: máy host thấy về điểm sinh còn máy kia thì không.
  > **`StartRun` đếm lại số người còn sống từ `PlayerRegistry` thay vì tin bộ đếm cũ.** Sau một ván thua bộ đếm đang ở 0; kẹp về tối thiểu 1 như trước thì ván chơi lại của hai người tưởng chỉ có một, và kết thúc ngay khi người đầu tiên gục.
  > **`GameEvents.Clear` KHÔNG được gọi giữa hai ván**, dù chú thích cũ ghi "gọi khi kết thúc ván" và tôi từng ghi nợ đúng như vậy. Chơi lại không nạp lại scene nên `HitFeedback` và `PlayerHud` vẫn sống và đã đăng ký từ `OnEnable`; gọi `Clear` sẽ gỡ luôn chúng và ván sau mất sạch phản hồi khi đánh trúng — mã vẫn chạy đúng nên rất khó truy. Người nghe của một ván đều tự gỡ trong `OnDisable`, không có rò rỉ nào ở ranh giới giữa hai ván. Đã sửa lại chú thích.
  > **Phím chơi lại dựng bằng mã, không lấy từ `LACControls`.** Tài sản đó bị nhân bản cho từng người chơi nên không bản nào là của giao diện; và `PlayerInputReader` bị tắt khi người chơi gục — đúng lúc cần bấm nhất thì nó không còn nghe. Nhận `R`, `Enter` và nút Start trên tay cầm.
  > **Không dùng TextMeshPro cho chữ tiếng Việt.** Vẽ sẵn ba cụm từ thành ảnh bằng bộ chữ pixel 5×8 — hai dòng trên cho dấu phụ, năm dòng giữa cho chữ cái, một dòng dưới cho dấu nặng. Giao diện hiện chỉ cần ba cụm nên ảnh vẽ sẵn rẻ hơn nhiều so với dựng cả một hệ thống phông, và tránh được đúng vấn đề đã ghi ở T-15: phông vector bị làm mờ khi thu về cỡ pixel art.
  > Tiến trình hiển thị bằng **một hàng 16 ô, mỗi ô một đợt** — cùng cách với thanh máu ở T-15B và cùng lý do: đếm được "qua 7 trên 16" nhanh hơn đọc một con số. Màu **không dùng nhóm son** dù đây là giao diện, vì nhóm son dành riêng cho đòn địch: thắng dùng hoè, thua dùng điệp.
  > **Một lỗi đọc hiểu sửa ngay sau khi chụp:** ô rỗng để màu `ThanDam` thì gần như đen trên nền mờ và cả hàng biến mất. Đổi sang `ThanNhat`.
  > Đã kiểm chứng khi chạy, một mạch không rời play mode: `Defeat` ở đợt 1 → màn hình hiện với đủ 16 ô → xin chơi lại → **`Defeat → WaveActive` đợt 1 với 6 quái và seed mới (16275085 → 16299656)** → dọn sạch đợt 1 → sang đợt 2 → `Defeat` lần hai. Người chơi được hồi đủ 10 máu và về đúng điểm sinh. Ép trạng thái `Victory` để kiểm màn hình thắng: chữ vàng, 16/16 ô đầy.

- [x] **T-15C** Sửa ba lỗi phát hiện khi chơi thử: nháy trúng đòn, quái vây xác, tầm đánh Gióng — @Kiet · 03/09
  > **Nháy sáng khi trúng đòn chưa từng hoạt động kể từ T-15.** `SpriteRenderer.color` được **nhân** vào màu điểm ảnh chứ không thay thế nó; `HitFeedback` truyền `Color.white`, tức nhân với 1, nên không có gì đổi trên màn hình. Mã vẫn chạy đúng, sự kiện vẫn phát, số sát thương vẫn hiện — chỉ riêng phần nhìn là không có. T-15 ghi "đã kiểm chứng" cho số sát thương và đẩy lùi nhưng **không kiểm riêng phần nháy**, nên lỗi lọt qua.
  > Sửa bằng hệ số nhân `(6, 6, 6)`: nhân quá 1 thì màu tràn trần và kẹp về trắng — thân loé trắng, nét viền tối vẫn giữ hình. Không cần shader riêng và giữ được gộp lệnh vẽ vì vẫn chỉ là màu đỉnh. Thời gian nháy 0.07 → 0.11 giây; dưới 0.08 giây thì mắt không kịp bắt khi có 40 quái trên sân. Giữ nguyên độ trong suốt hiện tại để nháy sáng không kéo người chơi đã gục hiện rõ trở lại.
  > **Vấp cùng một cái bẫy hai lần trong một phiên:** đổi giá trị mặc định trong mã không có tác dụng khi thành phần đã tồn tại trong scene hoặc prefab — giá trị đã tuần tự hoá đè lên. Phải sửa cả `HitFeedback` trong `Arena.unity`. Lần đầu là `WeaponAuto._tint` trong `Player.prefab`.
  > **Quái vây kín xác người chơi:** `PlayerRegistry.Nearest` không lọc người đã gục, nên cả đàn tiếp tục truy đuổi cái xác và người chơi còn sống thì không con nào để ý. Ở chơi đơn thì màn hình đứng lại với một đống quái chồng lên chỗ vừa ngã. Thêm `PlayerCharacter.IsAlive` (tham chiếu `PlayerHealth` lấy một lần trong `Awake` — 40 con hỏi mỗi khung thì `GetComponent` là thứ mục 5 cấm), `Nearest` bỏ qua người đã gục, quái không còn mục tiêu thì dừng lại và trở về dáng đứng yên.
  > **Tầm đánh Gióng — nguyên nhân thật không nằm ở tầm đánh.** Quái cơ bản chạy tốc độ 3, **đúng bằng Gióng**; nhân vật chậm nhất không bao giờ thoát được vòng vây nên tầm 2.5 thành vô dụng. Hạ tốc độ quái xuống 2.2 (khoảng 73% của nhân vật chậm nhất) để vị trí lấy lại ý nghĩa cho cả ba nhân vật, rồi mới nâng tầm Gióng 2.5 → 3.2. Không nâng cao hơn: 10 máu và sát thương 3 là bản sắc nhân vật cận chiến, biến Gióng thành tầm xa là xoá nhân vật. Sai lệch so với mục 1.2 đã ghi vào bảng ở mục 7 của CLAUDE.md.
  > Đã kiểm chứng khi chạy: đo được khoảng cách xa nhất Gióng đánh trúng là **3.20**, khớp giá trị đặt; sau khi người chơi gục thì đàn quái tản ra và đứng lại thay vì dồn vào; ép giữ màu nhân `(6,6,6)` và chụp ảnh — **quái hiện thành hình trắng đặc**, xác nhận cơ chế nháy đã hoạt động.
  > **Chưa kiểm chứng:** nháy sáng trong một trận thật, vì cả hai lần đo người chơi đều đã gục trước khi bộ thu thập kịp chạy. Cơ chế thì đã chứng minh; nhịp điệu của nó cần một lượt chơi bằng bàn phím.
